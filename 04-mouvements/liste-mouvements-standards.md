...
- **Astuce de repli — pose neutre récalcitrante** : si une pose "neutre/croisement" (ex : frame 3 d'un cycle de marche) échoue de façon répétée en miroir ou en génération directe (incohérences, accessoire qui disparaît, jambe qui ne s'inverse pas), la remplacer par **l'image de référence déjà validée du personnage** (posture debout par défaut). C'est une pose neutre par nature, zéro risque de nouvelle génération ratée, et elle fonctionne très bien comme frame de transition/repos dans un cycle à 4 temps.
